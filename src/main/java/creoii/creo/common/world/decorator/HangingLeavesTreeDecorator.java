package creoii.creo.common.world.decorator;

import com.mojang.serialization.Codec;
import com.mojang.serialization.codecs.RecordCodecBuilder;
import creoii.creo.common.block.HangingLeavesBlock;
import creoii.creo.core.registry.DecoratorRegistry;
        ...
        for (BlockPos pos : leavesPositions) {
            if (random.nextFloat() < this.probability) {
                if (Feature.isAir(world, pos.down()) && state.getBlock() instanceof HangingLeavesBlock) {
                    BlockState blockstate = this.state.with(HangingLeavesBlock.HALF, HangingLeavesBlock.Half.UPPER);
                    int length = random.nextInt(maxLength) + minLength;
                    for (int j = pos.down().getY(); j >= pos.down().getY() - length; --j) {
                        if (j == pos.down().getY() - length)
                            blockstate = blockstate.with(HangingLeavesBlock.HALF, HangingLeavesBlock.Half.LOWER);
